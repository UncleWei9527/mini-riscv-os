...
void task_init() {
  ...
  while (1) {
    ...
    if (dead_pid > 0) {
      printf("Init collect Zombie pid:%d ,ExitCode:%d\n", dead_pid, status);
    }
    delay(10000);
  }
}
...
int main() {
  ...
  int pid2 = create_task(task_func2);
  int pid3 = create_task(task_func3);
  task_pool[pid1 - 1].parent = &task_pool[init_pid - 1];
  task_pool[pid2 - 1].parent = &task_pool[init_pid - 1];
  task_pool[pid3 - 1].parent = &task_pool[init_pid - 1];
